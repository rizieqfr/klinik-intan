...
export default function PembayaranUgd({accessToken}) {
    ...
    const kolomPembayaran = [
        ...
        { header: 'Nama Pasien',accessorKey: 'fullname', },
        { header: 'Dokter',accessorKey: 'namaDokter', },
        { header: 'Diagnosis',accessorKey: 'diagnosa', },
        { header: 'Metode Pembayaran',accessorKey: 'pembayaran', },
        {
                ...
                <div>
                    {row.original.status === 'LUNAS' ? (
                        <td className='flex justify-center'>
                            ...
                            <Link href={`invoice/${row.original.id}`} className='bg-blue-600 text-white rounded px-[14px] py-[3px] font-semibold text-sm mr-2'>Cetak Invoice</Link>
                        </td>
                    ) : (
                        <td className='flex justify-center'>
                            ...
                            <Link href={`invoice/${row.original.id}`} disabled className='bg-blue-300 text-white rounded px-[14px] py-[3px] font-semibold text-sm mr-2'>Cetak Invoice</Link>
                        </td>
                    )}
                </div>
    ...
    const getDataPayment = async () => {
        try {
            const res = await ClientRequest.GetPayment(accessToken, 'ugd')
        ...
        <div className='bg-white flex gap-[32px] min-h-screen'>
            ...
            <div className='w-full pr-[32px]'>
                <div className='flex items-center justify-between pt-[40px] mb-8'>
                    <h1 className='text-4xl text-[#353A40] font-bold'>Pembayaran UGD</h1>
                    <h1>Navigasi / <span className='text-cyan font-medium'>Pembayaran UGD</span></h1>
                </div>
